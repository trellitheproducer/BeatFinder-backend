...
async def yt_search_query(client, q, yt_key):
    try:
        ...
        print("[Warn] query failed: " + q + " - " + str(e))
        return []


def extract_beat(item):
...
async def youtube_search(
    ...
    master_key = slug + "_master_" + CACHE_VER
    page_key   = slug + "_p" + str(page) + "_" + CACHE_VER
    ...
    if cached:
        ...
        return {"query": artist + " type beat", "total": len(cached), "beats": cached, "cached": True}
    ...
    if not master:
        ...
        async with httpx.AsyncClient(timeout=25.0) as client:
            ...
            if extra_queries:
                # Artist-specific custom queries (e.g. J.I)
                primary_queries = [q.strip() for q in extra_queries.split(",") if q.strip()]
                supplemental_queries = []
            else:
                # PRIMARY - proven existing structure preserved exactly
                primary_queries = [
                    artist + " type beat free",
                    artist + " type beat free instrumental 2024",
                    artist + " type beat free instrumental 2025",
                ]
                # SUPPLEMENTAL - added on top for extra coverage
                supplemental_queries = [
                    artist + " type beat",
                    artist + " instrumental",
                    artist + " instrumental type beat",
                ]

            # Run primary queries
            for q in primary_queries:
                items = await yt_search_query(client, q, YT_KEY)
                for item in items:
                    beat = extract_beat(item)
                    if not beat or beat["videoId"] in seen_ids:
                        continue
                    seen_ids.add(beat["videoId"])
                    all_beats.append(beat)
            print("[Primary]      " + str(len(all_beats)) + " beats")

            # Run supplemental queries
            for q in supplemental_queries:
                ...
                for item in items:
                    ...
                    seen_ids.add(beat["videoId"])
                    all_beats.append(beat)
            print("[Supplemental] " + str(len(all_beats)) + " beats total")

            # FALLBACK - only fires if still very few results
            if len(all_beats) < 20:
                print("[Fallback]     Only " + str(len(all_beats)) + " - trying fallback queries")
                for q in [artist + " beat", artist + " free beat"]:
                    items = await yt_search_query(client, q, YT_KEY)
                    for item in items:
                        beat = extract_beat(item)
                        if not beat or beat["videoId"] in seen_ids:
                        ...
                        seen_ids.add(beat["videoId"])
                        all_beats.append(beat)
                print("[Fallback]     " + str(len(all_beats)) + " beats total")

        master = all_beats
        await set_cached(db, master_key, master)
        print("[Cache SET]    " + master_key + " - " + str(len(master)) + " beats stored")
    ...
    return {"query": artist + " type beat", "total": len(beats), "beats": beats, "cached": False}
